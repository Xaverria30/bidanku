...
const JadwalSchema = Joi.object({
    ...
    jam_mulai: Joi.string().pattern(/^([01]?[0-9]|2[0-3]):[0-5][0-9](:[0-5][0-9])?$/).required().messages({ 'string.pattern.base': 'Jam mulai format HH:MM atau HH:MM:SS' }),
    jam_selesai: Joi.string().pattern(/^([01]?[0-9]|2[0-3]):[0-5][0-9](:[0-5][0-9])?$/).allow(null, '').optional().messages({ 'string.pattern.base': 'Jam selesai format HH:MM atau HH:MM:SS' })
}).unknown(false);
